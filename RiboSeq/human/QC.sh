...
echo "   human_tRNA"
# human_tRNA
mapped_log=${mapping_data}${id}/human_tRNA.Log.final.out
...
echo -e "${id}\tSTAR_human_tRNA\tuniq_map\t${uniq_map}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\tmulti_map\t${multi_map}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\tunmapped\t${unmapped}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\ttoo_many_loci\t${too_many_loci}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\ttoo_many_mismatches\t${too_many_mismatches}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\ttoo_short\t${too_short}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\tother\t${other}" >> $stats_out
echo -e "${id}\tSTAR_human_tRNA\tchimeric\t${chimeric}" >> $stats_out


echo "   human_cDNA"
# human_cDNA
mapped_log=${mapping_data}${id}/human_cDNA.Log.final.out
...
echo -e "${id}\tSTAR_human_cDNA\tuniq_map\t${uniq_map}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\tmulti_map\t${multi_map}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\tunmapped\t${unmapped}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\ttoo_many_loci\t${too_many_loci}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\ttoo_many_mismatches\t${too_many_mismatches}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\ttoo_short\t${too_short}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\tother\t${other}" >> $stats_out
echo -e "${id}\tSTAR_human_cDNA\tchimeric\t${chimeric}" >> $stats_out
...
cat $flagstat_split | while read line; do 
    ...
    # pattern between 'out.' and '.mate1' >> human_tRNA.Unmapped.out.AGCTA.mate1
...
cat $flagstat_split | while read line; do 
    ...
    # human_genome
    mapped_log=${RSEM_data}${id}/${bc}/human_genome.Log.final.out
...
cat $flagstat_split | while read line; do 
    ...
    bc=$(echo $line | grep -zoP "(?s)(?<=out[.])(.*)(?=[.]mate1)")
    dedup_log=${work_dir}RSEM/${id}/${bc}/human_genome.Aligned.toTranscriptome.dedup.out

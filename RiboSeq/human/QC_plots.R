...
### STAR human_rRNA, human_tRNA
STAR_df = stats_samples[stats_samples$Step %in% c('STAR_human_rRNA', 'STAR_human_tRNA'),]
...
### STAR human cDNA
STAR_cDNA_df = stats_samples[stats_samples$Step == 'STAR_human_cDNA',]
...
### STAR unmapped human tRNA >> what will map to genome
STAR_unmapped_human_tRNA_df = stats_samples[stats_samples$Type == 'splitseq_to_map',]
head(STAR_unmapped_human_tRNA_df); dim(STAR_unmapped_human_tRNA_df)
...
colsMap = c('grey', brewer.pal(n = 8, name = "Dark2")[c(1,3,4)], 'red')
names(colsMap) = c('unmapped', 'human_rRNA', 'human_tRNA', 'human_cDNA', 'human_genome')
colsMap
...
df_STAR_w_cDNA_overall = rbind(df_STAR_w_cDNA[df_STAR_w_cDNA$Type != 'unmapped',], df_STAR_w_cDNA[df_STAR_w_cDNA$Step == 'human_cDNA' & df_STAR_w_cDNA$Type == 'unmapped',])
...
df_STAR_w_cDNA_overall[df_STAR_w_cDNA_overall$Step == 'human_cDNA' & df_STAR_w_cDNA_overall$Type == 'unmapped', ]$Step = 'unmapped'
head(df_STAR_w_cDNA_overall); dim(df_STAR_w_cDNA_overall)
...
################################################
# STAR_human_tRNA unmapped == input for splitseq_to_map by barcode
head(STAR_unmapped_human_tRNA_df); dim(STAR_unmapped_human_tRNA_df)

max_y_axis_barcode = max(aggregate(STAR_unmapped_human_tRNA_df$Counts, by=list(STAR_unmapped_human_tRNA_df$Sample), FUN=sum)$x)
max_y_axis_barcode

colsBC = brewer.pal(n = length(unique(STAR_unmapped_human_tRNA_df$Step)), name = "Set3")[c(1:length(unique(STAR_unmapped_human_tRNA_df$Step)))]
names(colsBC) = unique(STAR_unmapped_human_tRNA_df$Step)
colsBC


print('03_SplitBarcode.pdf')
pdf(file=paste0(path_plot, '03_SplitBarcode.pdf'))
ggplot(STAR_unmapped_human_tRNA_df, aes(x=Sample, y=Counts, fill=Step)) +
    geom_bar(stat='identity') +
...
ggplot(STAR_unmapped_human_tRNA_df, aes(x=Step, y=Counts, fill=Step)) +
    geom_bar(stat='identity') +
    scale_fill_manual(values=colsBC) +
    my_theme + theme(legend.position="bottom",
    axis.title.x=element_blank(), legend.key.width= unit(2, "mm"), legend.key.size = unit(1, 'mm')) + guides(fill=guide_legend(nrow=1,byrow=TRUE)) +
...
################################################
# STAR_human genome == mapped per barcode
head(STAR_genome_df_simple); dim(STAR_genome_df_simple)
head(STAR_genome_df_detailed); dim(STAR_genome_df_detailed)
...
STAR_genome_df_simple_aggr = aggregate(STAR_genome_df_simple$Counts, by=list(STAR_genome_df_simple$Sample, STAR_genome_df_simple$Type), FUN=sum)
names(STAR_genome_df_simple_aggr) = c('Sample', 'Type', 'Counts')
STAR_genome_df_simple_aggr$Step = 'human_genome'
...
df_STAR_w_genome_overall = rbind(df_STAR_w_genome[df_STAR_w_genome$Type != 'unmapped',], df_STAR_w_genome[df_STAR_w_genome$Step == 'human_genome' & df_STAR_w_genome$Type == 'unmapped',])
...
df_STAR_w_genome_overall[df_STAR_w_genome_overall$Step == 'human_genome' & df_STAR_w_genome_overall$Type == 'unmapped', ]$Step = 'unmapped'
head(df_STAR_w_genome_overall); dim(df_STAR_w_genome_overall)
...
###
# 'uniq_map', 'multi_map', 'too_many_loci', 'too_many_mismatches', 'too_short', 'other', 'chimeric'
STAR_genome_df_detailed$Step = 'human_genome'
